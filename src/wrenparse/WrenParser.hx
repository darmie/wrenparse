...
class WrenParser extends hxparse.Parser<hxparse.LexerTokenSource<Token>, Token> implements hxparse.ParserBuilder {
	...
	function getExpr() {
		return switch stream {
			...
			// unary ops
			case [{tok: Binop(OpSub), pos: p}, exp = getExpr()]: {expr: EUnop(OpNeg, false, exp), pos: exp.pos};
			case [{tok: Unop(OpNegBits), pos: p}, exp = getExpr()]: {expr: EUnop(OpNegBits, false, exp), pos: exp.pos};
			...
			case EConst(constant):
				{
					...
					return switch stream {
											...
											if (peek(0).toString() != "<newline>") { // a.field()
												...
												switch stream {
													case [{tok: POpen, pos: pp}]: {
															...
															return {
																expr: ECall({expr: EField(parseExpr({expr: EConst(CIdent(a)), pos: p}), '$s.$a'), pos: p},
																	args),
																pos: pp
															};
														}
						...
						case _: {
								switch peek(0) { // buggy: it  captures white space as new lines sometimes;
									case {tok: Line}: {
											return e;
										}
		...
		switch stream {
			...
			case [{tok: BkOpen, pos: p4}]:
				{ // list
					...
					exp = {expr: EArrayDecl(exps), pos: p4};
					pos = p4;
				}
			case [interval = parseInterval()]:
				{
